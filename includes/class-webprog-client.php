...
class TTB_WebProg_Client {
  ...
  public static function render($token) {
    ...
    // Calcular fecha mínima: próximo lunes-viernes a partir de hoy +3 días laborables
          ...
          <div style="border-radius:14px;border:2px dashed var(--ttb-border);background:linear-gradient(135deg,#f0f9ff 0%,#fff 100%);padding:48px 32px;text-align:center">
            ...
            <a href="<?php echo esc_url($project->web_url); ?>" target="_blank" rel="noopener"
               style="display:inline-flex;align-items:center;gap:10px;
                      ...
                      padding:18px 40px;border-radius:14px;box-shadow:0 8px 24px rgba(215,33,115,.30)">
              🔗 Abrir mi web
            </a>
          ...
          <div class="ttbwp-panel" id="ttbwp-panel-accept">
            ...
            <div style="background:#fff7ed;border:1.5px solid #fed7aa;border-radius:14px;padding:18px 22px;margin-bottom:20px">
              ...
              <p style="margin:0;font-size:14px;color:#c2410c;line-height:1.6">
                El día que elijamos para subir la web a producción, <strong>la web estará parcialmente caída o sin funcionalidad</strong> durante unas horas mientras realizamos el proceso de subida. Por eso te pedimos que elijas un día de lunes a viernes que sea conveniente para ti.
              </p>
            </div>
            ...
            <form method="post" action="">
              ...
              <div style="margin-bottom:20px">
                <label style="display:block;font-weight:700;font-size:15px;color:var(--ttb-text);margin-bottom:8px">
                  📅 Fecha preferida de subida <span style="color:#e11d48">*</span>
                </label>
                ...
                >
                <p id="ttbwp-date-error" style="display:none;margin:6px 0 0;font-size:13px;font-weight:700;color:#e11d48">
              ...
              <div style="background:#ecfdf5;border:1.5px solid #6ee7b7;border-radius:14px;padding:16px 20px;margin-bottom:20px">
                <p style="margin:0;font-size:14px;color:#065f46;line-height:1.6">
                  Al aceptar confirmas que la web está correcta tal y como está y autorizas a TicTac a proceder con la subida a producción en la fecha indicada. Recibirás una confirmación por email.
                </p>
              </div>

              <div class="ttb-actions">
                <button class="ttb-btn" type="submit" id="ttbwp-accept-btn" style="background:linear-gradient(135deg,#10b981,#059669)">
          ...
          <div class="ttbwp-panel" id="ttbwp-panel-changes" style="display:none">
            ...
            <form method="post" action="" enctype="multipart/form-data" id="ttbwp-changes-form">
              ...
              <div style="display:flex;gap:10px;flex-wrap:wrap;margin:16px 0 24px">
                ...
                <button type="button" class="ttb-btn ttb-btn--ghost" id="ttbwp-add-image">🖼️ Añadir captura con comentario</button>
              </div>
                      ...
                      <?php foreach ($blocks as $bl): ?>
                        ...
                        <?php elseif (($bl['type'] ?? '') === 'image'): ?>
                          <div style="background:#fff;border-radius:10px;border:1px solid rgba(0,0,0,.07);overflow:hidden">
                            <?php if (!empty($bl['image_url'])): ?><a href="<?php echo esc_url($bl['image_url']); ?>" target="_blank"><img src="<?php echo esc_url($bl['image_url']); ?>" style="width:100%;max-height:320px;object-fit:contain;display:block;background:#f4f4f4" alt="Adjunto"></a><?php endif; ?>
                  ...
                  <?php else: ?>
                    ...
                    <?php $old_images = json_decode((string)$rev->images, true); if (is_array($old_images) && $old_images): ?>
                      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:10px">
                        <?php foreach ($old_images as $img_url): ?><a href="<?php echo esc_url($img_url); ?>" target="_blank"><img src="<?php echo esc_url($img_url); ?>" style="height:72px;width:auto;border-radius:8px;border:1px solid rgba(0,0,0,.1);object-fit:cover" alt="Adjunto"></a><?php endforeach; ?>
                      </div>
                    <?php endif; ?>
                  <?php endif; ?>
                <?php endif; ?>
    ...
    .ttbwp-block { border:1.5px solid var(--ttb-border);border-radius:14px;background:#fff;margin-bottom:14px;overflow:hidden;transition:box-shadow .2s; }
    .ttbwp-block:focus-within { box-shadow:0 0 0 3px rgba(215,33,115,.12);border-color:rgba(215,33,115,.4); }
    .ttbwp-block-header { display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#f9fafb;border-bottom:1px solid var(--ttb-border);gap:8px; }
    ...
    .ttbwp-block-btn { background:none;border:1px solid var(--ttb-border);border-radius:8px;padding:4px 8px;font-size:13px;cursor:pointer;color:var(--ttb-muted);transition:background .15s,color .15s; }
    ...
    .ttbwp-wysiwyg-bar { display:flex;flex-wrap:wrap;gap:2px;padding:8px 10px;background:#f9fafb;border-bottom:1px solid var(--ttb-border); }
    .ttbwp-wysiwyg-bar button { background:none;border:1px solid transparent;border-radius:6px;padding:4px 8px;font-size:13px;font-weight:700;cursor:pointer;color:var(--ttb-text);line-height:1.4;min-width:28px;transition:background .12s,border-color .12s; }
    ...
    .ttbwp-editor { min-height:120px;padding:14px 16px;outline:none;font-size:15px;line-height:1.7;color:var(--ttb-text); }
    ...
    .ttbwp-img-dropzone { border:2px dashed var(--ttb-border);border-radius:12px;padding:28px 20px;text-align:center;cursor:pointer;background:#fafafa;margin-bottom:12px;transition:border-color .2s,background .2s; }
    ...
    .ttbwp-img-preview-remove { position:absolute;top:-8px;right:-8px;background:#e11d48;color:#fff;border:none;border-radius:50%;width:22px;height:22px;font-size:12px;font-weight:900;cursor:pointer;line-height:1; }
    .ttbwp-img-caption { width:100%;border:1px solid var(--ttb-border);border-radius:10px;padding:10px 12px;font-size:14px;line-height:1.5;resize:vertical;min-height:72px;font-family:inherit;color:var(--ttb-text);outline:none;transition:border-color .2s,box-shadow .2s; }
    ...
    (function(){
      ...
      function isWeekend(dateStr) {
        ...
        var d = new Date(dateStr + 'T12:00:00'); // T12 para evitar problemas de zona horaria
      ...
      if (dateInput) {
        ...
        // Bloquear submit si fin de semana (seguridad extra)
        dateInput.closest('form').addEventListener('submit', function(e) {
          ...
          if (!dateInput.value) {
            e.preventDefault();
            dateError.textContent   = 'Por favor selecciona una fecha de subida.';
            dateError.style.display = 'block';
          }
        });
      }
      ...
      function addTextBlock(initialHtml) {
        ...
        div.innerHTML =
          ...
          + '<button type="button" class="ttbwp-block-btn" data-move="up">↑</button>'
          + '<button type="button" class="ttbwp-block-btn" data-move="down">↓</button>'
          + '<button type="button" class="ttbwp-block-btn ttbwp-block-btn--delete" data-delete="1">✕</button>'
          + '</div></div>'
        ...
        div.querySelectorAll('.ttbwp-wysiwyg-bar button').forEach(function(btn){
          btn.addEventListener('mousedown', function(e){
            ...
            if (cmd.indexOf('|') !== -1) { var parts=cmd.split('|'); document.execCommand(parts[0],false,parts[1]); }
      ...
      function addImageBlock(initialSrc, initialCaption) {
        ...
        div.innerHTML =
          ...
          + '<p style="margin:0 0 4px;font-weight:700;color:var(--ttb-text);font-size:14px">Arrastra una captura o haz clic</p>'
          + '<p style="margin:0;font-size:12px;color:var(--ttb-muted)">PNG, JPG, GIF, WEBP · Máx. ' + MAX_MB + ' MB</p>'
          + '<input type="file" accept="image/*" style="display:none">'
          + '</div>'
          + '<textarea class="ttbwp-img-caption" placeholder="Explica qué hay que cambiar..."></textarea>'
          + '</div>';
        getContainer().appendChild(div);
        var dz=div.querySelector('.ttbwp-img-dropzone'), input=div.querySelector('input[type=file]'), caption=div.querySelector('.ttbwp-img-caption');
        ...
        dz.addEventListener('drop',function(e){e.preventDefault();dz.classList.remove('dragover');var f=e.dataTransfer.files[0];if(f)loadFile(div,dz,f);});
        input.addEventListener('change',function(){if(input.files[0])loadFile(div,dz,input.files[0]);});
        bindBlockActions(div);
      }
      ...
      function showPreview(block,dz,src){
        ...
        var existing=block.querySelector('.ttbwp-img-preview'); if(existing)existing.remove();
        var wrap=document.createElement('div'); wrap.className='ttbwp-img-preview';
        var img=document.createElement('img'); img.src=src;
        var rm=document.createElement('button'); rm.type='button'; rm.className='ttbwp-img-preview-remove'; rm.textContent='✕';
        rm.addEventListener('click',function(){wrap.remove();block.removeAttribute('data-file-dataurl');block.removeAttribute('data-file-name');dz.style.display='';});
      ...
      function bindBlockActions(block){
        block.querySelector('[data-delete]').addEventListener('click',function(){if(getContainer().querySelectorAll('.ttbwp-block').length<=1){alert('Debe haber al menos un bloque.');return;}block.remove();});
      ...
      // ── Envío formulario cambios ──
      document.getElementById('ttbwp-changes-form').addEventListener('submit',function(e){
        e.preventDefault();
        var blocks=[],imageFiles=[],container=getContainer(),blockEls=container.querySelectorAll('.ttbwp-block'),hasContent=false;
        blockEls.forEach(function(bl,idx){
          var type=bl.getAttribute('data-type');
          if(type==='text'){var html=bl.querySelector('.ttbwp-editor').innerHTML.trim(),plain=bl.querySelector('.ttbwp-editor').innerText.trim();if(plain)hasContent=true;blocks.push({type:'text',html:html,idx:idx});}
          else if(type==='image'){var caption=bl.querySelector('.ttbwp-img-caption').value.trim(),dataUrl=bl.getAttribute('data-file-dataurl')||'',fname=bl.getAttribute('data-file-name')||'',ftype=bl.getAttribute('data-file-type')||'';if(dataUrl||caption)hasContent=true;blocks.push({type:'image',caption:caption,fileIndex:dataUrl?imageFiles.length:-1,idx:idx});if(dataUrl)imageFiles.push({blockIndex:blocks.length-1,dataUrl:dataUrl,name:fname,mimeType:ftype});}
        });
        ...
        imageFiles.forEach(function(f,i){var arr=f.dataUrl.split(','),mime=arr[0].match(/:(.*?);/)[1],bstr=atob(arr[1]),n=bstr.length,u8=new Uint8Array(n);for(var j=0;j<n;j++)u8[j]=bstr.charCodeAt(j);var blob=new Blob([u8],{type:mime});fd.append('ttbwp_img_file_'+i,blob,f.name||('captura-'+(i+1)+'.jpg'));});
        ...
        fetch(window.location.href,{method:'POST',body:fd}).then(function(r){return r.text();}).then(function(html){var match=html.match(/window\.location\.replace\((.+?)\)/);if(match)window.location.replace(JSON.parse(match[1]));else window.location.reload();}).catch(function(){btn.disabled=false;btn.textContent='📨 Enviar cambios';alert('Error al enviar. Inténtalo de nuevo.');});
      });
    })();
    </script>
    <?php
  ...
  /**
   * Devuelve el próximo día laborable (L-V) a partir de un timestamp dado.
   */
  private static function next_workday($ts) {
    $day = (int)date('N', $ts); // 1=lun … 7=dom
    if ($day === 6) $ts += 2 * 86400; // sáb → lun
    if ($day === 7) $ts += 1 * 86400; // dom → lun
  ...
  private static function handle_submit($project) {
    ...
    if ($action === 'accept') {
      ...
      // Recoger y validar fecha de subida
      $raw_date    = sanitize_text_field($_POST['go_live_date'] ?? '');
      $go_live_date = null;

      if ($raw_date) {
        $ts  = strtotime($raw_date);
        $day = $ts ? (int)date('N', $ts) : 0; // 1=lun…7=dom
        // Validar: fecha futura + L-V
      ...
      // Recargar proyecto con la nueva fecha para pasarla al mailer
      ...
      TTB_WebProg_DB::log($project->id, 'web_accepted', 'client', [
        'round'         => $round,
        'go_live_date'  => $go_live_date ?? 'no indicada',
      ]);
      ...
      for ($i = 0; $i < $img_count; $i++) {
        ...
        $att_id = media_handle_sideload([
          'name' => $_FILES[$key]['name'], 'type' => $type,
          'tmp_name' => $_FILES[$key]['tmp_name'], 'error' => $_FILES[$key]['error'], 'size' => $_FILES[$key]['size'],
        ], 0, null, ['post_title' => 'WebProg - ' . $project->name . ' #' . $round . ' img' . ($i+1), 'post_status' => 'private']);
        if (!is_wp_error($att_id)) { $uploaded[$i] = wp_get_attachment_url($att_id); $uploaded_count++; }
      }
      ...
      $wpdb->insert($revisions_table, [
        'project_id' => $project->id, 'round' => $round, 'type' => 'change',
        'message'    => trim($message_plain),
        'images'     => wp_json_encode($sanitized_blocks, JSON_UNESCAPED_UNICODE),
        'created_at' => TTB_WebProg_DB::now(),
      ]);

      $revision = $wpdb->get_row($wpdb->prepare("SELECT * FROM $revisions_table WHERE id=%d", $wpdb->insert_id));
      (new TTB_WebProg_Mailer())->send_changes_alert($project, $revision);

      TTB_WebProg_DB::log($project->id, 'changes_requested', 'client', [
        'round' => $round, 'text_blocks' => count(array_filter($sanitized_blocks, fn($b) => $b['type'] === 'text')), 'image_blocks' => $uploaded_count,
      ]);
      TTB_WebProg_DB::log($project->id, 'email_changes_sent', 'system', ['round' => $round, 'recipients' => 'hola + produccion']);
