...
if (class_exists('TTB_WebProg_Mailer')) return;

/**
 * TTB_WebProg_Mailer
 * Emails del módulo Revisiones Prog. Web
 * v2: emails usan ttb_webprog_entry=TOKEN para llevar al portal con pestañas
 */
class TTB_WebProg_Mailer {
  ...
  /* ─────────────────────────────────────────────
     ...
     Si tiene sesión → /briefing?ctab=web
     Si no tiene sesión → autologin con ctab=web
     Fallback → /briefing?webprog=TOKEN
  ───────────────────────────────────────────── */
  private function smart_url($token) {
    return home_url('/briefing?ttb_webprog_entry=' . urlencode($token));
  }

  /* ─────────────────────────────────────────────
     EMAIL AL CLIENTE: invitación a revisar la web
  ───────────────────────────────────────────── */
  public function send_review_invitation($project) {
    ...
    $subject   = get_option('ttb_webprog_email_subject', '🌐 Tu web está lista para revisar — TicTac Comunicación');
    $intro     = get_option('ttb_webprog_email_intro',   'Hemos programado tu web y ya está disponible para que la revises. Accede al enlace, navégala con calma y danos tu feedback.');
    $btn_label = get_option('ttb_webprog_email_btn',     'Ver mi web →');

    $message = $this->tpl_invitation($project->name, $url, $subject, $intro, $btn_label, $project->web_url);
  ...
  public function send_changes_alert($project, $revision) {
    ...
    $subject = '✏️ Cambios web solicitados (ronda ' . $round . ') — ' . $project->name;
  ...
  /* ─────────────────────────────────────────────
     TEMPLATES
  ───────────────────────────────────────────── */
  private function tpl_invitation($name, $url, $subject, $intro, $btn_label, $web_url) {
    $pink = $this->pink;
    $logo = $this->logo;
    ...
    if ($web_url) {
      $web_preview = '
        <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:14px 18px;margin:0 0 24px;word-break:break-all">
          <p style="margin:0 0 4px;font-size:11px;font-weight:900;color:#9ca3af;text-transform:uppercase;letter-spacing:.06em">🌐 URL de tu web</p>
          <a href="' . esc_url($web_url) . '" target="_blank" style="color:' . $pink . ';font-size:14px;font-weight:700;text-decoration:none">' . esc_html($web_url) . '</a>
        </div>';
    }
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:20px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.12)">
    <tr>
      <td align="center" style="background:linear-gradient(135deg,' . $pink . ' 0%,#a8005a 100%);padding:40px 32px 32px">
        ...
        <h1 style="margin:0 0 8px;color:#fff;font-size:22px;font-weight:900">🌐 Tu web está lista</h1>
        <p style="margin:0;color:rgba(255,255,255,.85);font-size:15px">Ya puedes verla y darnos tu feedback</p>
      </td>
    </tr>
    <tr>
      <td style="background:#fff;padding:36px 40px">
        ...
        <p style="margin:0 0 24px;font-size:15px;color:#4b5563;line-height:1.6">' . esc_html($intro) . '</p>
        ' . $web_preview . '
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;border-radius:12px;margin-bottom:24px">
          <tr>
            <td style="padding:16px 20px">
              <p style="margin:0 0 8px;font-size:12px;font-weight:900;color:#374151;text-transform:uppercase;letter-spacing:.06em">💡 Cómo revisar tu web</p>
              <p style="margin:0 0 5px;font-size:14px;color:#4b5563;line-height:1.5">✔️ &nbsp;Navega por todas las páginas y secciones con calma.</p>
              <p style="margin:0 0 5px;font-size:14px;color:#4b5563;line-height:1.5">✔️ &nbsp;Revísala en móvil y en ordenador si puedes.</p>
              <p style="margin:0;font-size:14px;color:#4b5563;line-height:1.5">✔️ &nbsp;Usa el botón del portal para aceptar o pedir cambios.</p>
            </td>
          </tr>
        </table>
      ...
      $go_live_block = '
        <div style="background:#fffbeb;border:1.5px solid #fcd34d;border-radius:14px;padding:18px 22px;margin-bottom:24px">
          ...
          <p style="margin:6px 0 0;font-size:13px;color:#b45309;line-height:1.5">
            El cliente ha sido informado de que la web estará parcialmente caída ese día.
          </p>
        </div>';
    }
  ...
  <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;border-radius:20px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.12)">
    ...
    <tr><td style="background:#fff;padding:32px 36px">
      ...
      <div style="background:#f9fafb;border-radius:12px;padding:18px 22px;margin-bottom:24px">
        ...
        <p style="margin:0 0 6px;font-size:14px;color:#1a1a2e"><strong>Web:</strong> <a href="' . esc_url($project->web_url) . '" style="color:' . $pink . '">' . esc_html($project->web_url) . '</a></p>
  ...
  <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;border-radius:20px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.12)">
    ...
    <tr><td style="background:#fff;padding:32px 36px">
      <div style="background:#fffbeb;border:1.5px solid #fcd34d;border-radius:14px;padding:20px 24px;margin-bottom:24px">
        <p style="margin:0 0 4px;font-size:20px;font-weight:900;color:#92400e">✏️ Cambios web solicitados — Ronda ' . $round . '</p>
        <p style="margin:0;font-size:14px;color:#b45309">El cliente ha pedido modificaciones en la programación web.</p>
      </div>
      <div style="background:#f9fafb;border-radius:12px;padding:18px 22px;margin-bottom:24px">
        <p style="margin:0 0 6px;font-size:14px;color:#1a1a2e"><strong>Cliente:</strong> ' . esc_html($project->name) . '</p>
        <p style="margin:0;font-size:14px;color:#1a1a2e"><strong>Web:</strong> <a href="' . esc_url($project->web_url) . '" style="color:' . $pink . '">' . esc_html($project->web_url) . '</a></p>
      </div>
