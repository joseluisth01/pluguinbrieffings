...
/**
 * portal-shell-webrev.php
 * Shell para la vista pública del cliente de revisiones web.
 * No requiere login — usa magic link con token.
 */

$token = sanitize_text_field($_GET['webrev'] ?? '');
...
<title>Revisión de diseño — TicTac Comunicación</title>
...
// Flash de operaciones POST
$flash = get_transient('ttb_webrev_flash_' . $token);
if ($flash) {
  delete_transient('ttb_webrev_flash_' . $token);
  $cls = $flash['type'] === 'success' ? 'ttb-flash--success' : 'ttb-flash--error';
  echo '<div class="ttb-flash ' . $cls . '">' . esc_html($flash['text']) . '</div>';
}

// Renderizar la vista de cliente
